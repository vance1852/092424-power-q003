...
- 送出线路停运或降容事件按 UTC 半开区间 `[starts_at, ends_at)` 生效，日分配按秒加权计算实际可用能力：重叠限制取最严格值（最小值）而非连乘，边界相接不重复扣减，跨午夜检修拆分到两个服务日，零能力窗口按其时长清零；同一事件重复登记会被拒绝，分配结果保留每条限制的来源与分时段能力；
- 每个（送出线路，服务日）只确认一份发电计划：同输入重算复用原结果（`replayed=true`），限制事件或提名修订使输入摘要变化时返回冲突，不会静默改写已确认计划；
